...
- `[Arch]` Fase P — Backend real con Supabase (env-gated, fallback al modo local).
  ...
  - **`lib/auth.ts` refactorizado** — Mismo contrato público (`signIn`, `signOut`, `loadCurrentUser`, `coursesAssignedTo`) pero **todas las funciones son async** y despachan internamente entre backend local y Supabase. El tipo de retorno de `signIn` pasa a ser `SignInResult = { kind: 'signed-in', user } | { kind: 'magic-link-sent', email }` para que la UI pueda distinguir el caso del magic link de Supabase del sign-in inmediato del backend local.
  - **`AuthContext.tsx` async + status loading** — Tres estados `loading | authenticated | unauthenticated` para diferenciar el primer fetch de sesión (Supabase puede tardar ~200 ms) de la ausencia real de sesión. Si Supabase está activo, suscribe a `supabase.auth.onAuthStateChange` para reaccionar a callbacks de magic link (alumno pulsa el enlace del email, vuelve al sitio, AuthContext detecta la sesión nueva via el listener) y a sign-out cross-tab. Expone `backendKind: 'local' | 'supabase'` para UI condicional.
  - **`LoginPage` adaptada** — En backend local, formulario email + nombre opcional con botón "Entrar". En backend Supabase, formulario solo email con botón "Enviarme un enlace"; tras submit, pantalla "Revisa tu email" con la dirección destino y opción "Usar otra dirección". Errores (red caída, email inválido) se muestran como `<Callout kind="warning">`. Loading state durante el `comprobando sesión…` del primer fetch.
  - **`supabase/schema.sql` (nuevo)** — Schema idempotente (`IF NOT EXISTS`) con cuatro tablas: `user_profile` (espejo de `auth.users` con `display_name`), `course_enrollment` (asignación user ↔ curso con `expires_at` opcional), `user_progress` (espejo del localStorage `{user_id, course_slug, storage_key, value JSONB}`), `exam_attempt` (intentos con `verification_id` único). RLS activado en las cuatro tablas con policies "only own data" + una policy especial `public verify by id` para que visitantes anónimos puedan leer un intento por `verification_id`. Trigger `on_auth_user_created` que rellena `user_profile` automáticamente al crear un user.
  - **`supabase/README.md` (nuevo)** — Runbook completo: setup del proyecto, aplicación del schema, configuración del provider de auth, env vars, asignación de cursos a usuarios (insert manual en `course_enrollment`), convención de `storage_key` (sin el prefijo `pv-learn-{slug}-` porque `course_slug` ya está en columna), endpoint público de verificación, migración del localStorage existente, modelo de costes.
  - **`pages/VerifyPage.tsx` (nuevo)** + ruta pública **`/cert/:verificationId`** — Endpoint público de verificación del certificado, accesible sin sesión (gracias a la policy SELECT anónima en `exam_attempt`). Cinco estados de UI: `loading`, `ok` (renderiza tarjeta con datos del intento), `not-found`, `disabled` (Supabase no configurado en este deployment — mensaje claro para el admin) y `error`. Componentizado para no asumir formato del intento. La página se monta con shell minimal (sin sidebar) para que la URL sea share-friendly.
  - **`AppShell.minimal`** ahora también incluye `/cert/*` además de `/login` y "sin sesión".
  - **`platform/.env.example`** con plantilla de env vars (`VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, y placeholders para `VITE_SENTRY_DSN` y PostHog que llegarán en Fase R).
  - **Pendiente para una sub-fase futura (P.6 — write-through sync)**: el progreso del alumno (notas, quiz, lab, exam, srs, highlights) sigue escribiendo solo a `localStorage`. Cuando se enchufe el backend real, hay que añadir una capa de sync que haga UPSERT a `user_progress` en background (write-through cache, offline-first). El roadmap completo está en `docs/backend-integration.md` § "Roadmap de migración".
- `[Tests]` 58 unit tests (incluye 3 nuevos para `lib/supabase` + auth refactorizado a async). tsc clean. Build OK. Bundle inicial **217 KB gzip** (mejora vs 247 KB en O.5; la lazy de las funciones supabase y la reorganización del auth liberó margen).


- `[Test]` Fase O — Production hardening: tests + CI + budget + error boundary.
  - **O.1 — Vitest + 55 unit tests**. Setup minimal con `happy-dom` (más rápido que jsdom). Cubre `lib/srs` (SM-2 completo: createCard, review con quality 0–5, isDue, humanizeInterval, saturación a MIN_EASE), `lib/storage` (prefijado de keys per-curso y globales), `lib/auth` (signIn/signOut/loadCurrentUser, persistencia, trim, manejo de storage corrupto), `lib/search` (highlight con normalización de tildes, case-insensitive, merge de rangos solapados), `lib/highlights` (buildPlainTextMap, locatePayload, wrapRangeWithHighlight, paint/unpaint roundtrip, highlightFromSelection), `lib/confetti` (smoke + prefers-reduced-motion), y `components/ErrorBoundary`. `vitest.config.ts` separado de `vite.config.ts` para no arrastrar plugins de runtime (PWA, Tailwind). Scripts `test:unit`, `test:unit:watch`, `test` (combina unit + exam).
  - **O.2 — Playwright + E2E**. `playwright.config.ts` con Chromium-only para CI rápido. `e2e/critical-flow.spec.ts` con 7 tests del flujo crítico: login local arranca sesión, header muestra título del curso, navega a teoría, cambia entre tabs, atajo `?` abre modal de ayuda, atajo `i` aplica `data-reading-mode`, logout desde ajustes redirige a `/login`. `webServer` autoarranca `npm run dev` y lo reutiliza si ya está. `e2e/README.md` con instrucciones.
  - **O.3 — GitHub Actions CI** (`.github/workflows/ci.yml`). Cinco jobs paralelos en cada PR y push a main: `validate` (Python validador para agent365-cert + demo-pv-learn), `lint-and-typecheck` (tsc + ESLint, ESLint non-blocking por ahora), `unit-tests` (vitest + test:exam), `e2e-tests` (Playwright en Chromium, sube traces como artifact en fallo), `build-and-budget` (build de producción + check-bundle-budget). `concurrency` cancela runs anteriores del mismo PR al empujar nuevo commit. Permission scopes mínimos. Caché de `node_modules` en cada job vía `actions/setup-node@v4`.
  - **O.4 — ErrorBoundary global**. `components/ErrorBoundary.tsx` (clase porque `componentDidCatch` no tiene equivalente en hooks) envuelve TODO el árbol del router. Captura crashes de chunks lazy, librerías que tiran, render condicional con undefined. Muestra una pantalla amable con dos acciones (Recargar / Volver al inicio) y, en `import.meta.env.DEV`, un `<details>` con el stack completo y `componentStack` de React. Cuando esté Sentry (env-gated, Fase R), llama a `window.Sentry.captureException(error, { contexts: { react } })`. Cobertura: 3 tests con React Testing Library (renderiza children sin error, captura y muestra fallback, llama a Sentry si existe).
  - **O.5 — Bundle-size budget**. `scripts/check-bundle-budget.mjs` valida que `index-*.js` ≤ **290 kB gzip** y `index-*.css` ≤ **30 kB gzip**. Hoy en 247 KB JS / 16 KB CSS, con 43 KB de headroom. Si futuras fases dejan algo no-lazy o añaden librerías core sin justificación, CI falla. Los chunks lazy (`ModulePage`, `RepasoPage`, mermaid, cytoscape, katex) NO se cuentan: su tamaño es coste deferred.
- `[Build]` Validador 277 OK · agent365-cert + 34/1warn/0 demo-pv-learn. tsc clean. Build OK. test:unit **55/55**. test:exam **34/34**. Bundle budget OK.
...
- `[Arch]` Fase N.1 — Refactor multi-curso (Fase 8 del plan original). Rutas canónicas `/cursos/<slug>/...`, registry dinámico de cursos, storage prefijado por slug.
  ...
  - **`App.tsx`** — `AppShell` deriva el slug activo de `useLocation().pathname` y monta `<CourseProvider slug={activeSlug}>` por encima de Header, NavSidebar y Routes — así cualquier componente del shell puede usar `useCourse()`. Las rutas se mueven a `/cursos/:slug/{home,progreso,modulo/:id/:section,examen,certificado/:attemptId,repaso}`. Helpers `DefaultCourseRedirect`, `LegacyModuleRedirect`, `LegacyCertificateRedirect` mantienen retrocompatibilidad: cualquier bookmark previo (`/modulo/1/teoria`, `/progreso`, `/examen`, `/repaso`, `/`) redirige al curso por defecto.
  - **`lib/course.ts` re-escrito como delegador del registry** — Mantiene los exports estáticos (`COURSE_TITLE`, `AREAS`, `MODULES`, `CONTENT_MODULES`, `EXAM_MODULE`, `findModule`, etc.) por retrocompatibilidad con componentes no refactorizados, pero los valores vienen del `defaultCourseSlug()` del registry. Mantiene un fallback estático de `AREAS` por si el registry no encuentra cursos (Node tests).
  - **`lib/storage.ts` (nuevo)** — Centraliza las convenciones de storage keys: `pv-learn-{slug}-{detail}` para datos del curso (notas, quizzes, labs, exámenes, highlights, flashcards, progress) y `pv-learn-{detail}` para preferencias globales (modo lectura, contadores del Pomodoro, nombre del alumno). Expone el hook `useCourseStorageKey(detail)` y un slug activo trackeado a nivel de módulo (`setActiveCourseSlug` / `activeCourseSlug()`) para que código fuera de React (lib/progress, lib/highlights) construya las keys correctas. CourseProvider sincroniza ese slug en un `useEffect`.
  - **Refactor masivo de hooks/components** que usaban `agent365-…` keys:
    - **Per-curso** (`pv-learn-{slug}-{detail}`): `useNotes`, `useQuizState` (history + cooldowns), `useLabState`, `useExamState`, `useFlashcards`, `lib/highlights`, `lib/progress` (reading, quiz-history, section-visits, access-mode).
    - **Globales** (`pv-learn-{detail}`): `useReadingMode`, `focusStore` (4 keys), `Certificate.tsx` (learner name).
  - **Refactor de paths internos**: `Header`, `NavSidebar` (AreaGroup, ExamSection, RepasoLink), `ModuleRow` (ambas variantes), `ModulePage` (Navigate redirects, section tabs, prev/next), `ProgressPage` (NextStepCard, ModuleRow inline), `Resources` (cross-references), `Certificate`, `CertificatePage`, `RepasoPage`, `HomePage`. Todos consumen `useCourse().href(path)` para construir URLs relativas al curso activo. `lib/search.ts` devuelve `to` sin `/cursos/<slug>/` y `SearchPalette` prefija via `href()`.
  - **CustomEvent renombrado**: `agent365-progress-changed` → `pv-learn-progress-changed` (actualizado en `lib/progress`, `useQuizState`, `ScrollProgress`, `SettingsPage`).
  - **Reset de progreso legacy**: según decisión del usuario (audiencia interna nueva), las claves `agent365-*` no se migran. Quedan como huérfanas en `localStorage` y se pueden purgar manualmente. La nueva plataforma usa exclusivamente `pv-learn-*`.
  - **Auth + catálogo (Fase 9)**: en el modelo final, `/` mostrará un catálogo filtrado por los cursos asignados al alumno tras login. Hoy `/` redirige al único curso disponible (`/cursos/agent365-cert/`). Cuando llegue auth, la única zona a tocar es el componente que vive en `/` (los cursos individuales ya están separados).
- `[Build]` Validador 277 OK / 0 warnings / 0 errors. tsc clean. Build OK (PWA: 159 precache entries). test:exam 34/34 OK. Smoke test de rutas: `/`, `/cursos/<slug>/`, `/cursos/<slug>/modulo/1/teoria` y `/modulo/1/teoria` (legacy) responden 200 con redirect correcto.
